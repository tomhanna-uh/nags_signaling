# =============================================================================
# 05_model_paper2_h14_bandwidth_interaction.R
# Tier 1 Model for Paper 2 (Ideological Commitment Signaling)
# H14: Larger domestic opposition increases NAG support to non-democracy targets (with bandwidth interaction)
# =============================================================================
here::i_am("R/models/05_model_paper2_h14_bandwidth_interaction.R")
...
df_model <- df_final |>
  ...
  mutate(
    ...
    NAGTargetsNonDemocracy  = 1 - nags_targets_democracy,
...
# ----------------------------------------------------------------------------
# Primary model: Quasi-Poisson with triple interaction (standard * syntax)
# ----------------------------------------------------------------------------
model_h14_qp <- feglm(
...
# Save tables
# ----------------------------------------------------------------------------
if (!is.null(model_h14_nb)) {
  etable(model_h14_qp, model_h14_nb,
         file = here("results/tables", "paper2_h14_quasipoisson_nb.tex"),
         replace = TRUE)
  coef_table <- etable(model_h14_qp, model_h14_nb, tex = FALSE)
} else {
  etable(model_h14_qp,
         file = here("results/tables", "paper2_h14_quasipoisson.tex"),
         replace = TRUE)
  coef_table <- etable(model_h14_qp, tex = FALSE)
}

write_csv(coef_table, here("results/tables", "paper2_h14_coefficients.csv"))
...
preds <- predict(model_h14_qp, newdata = newdata, type = "response")
...
print("Predicted Probabilities Table (H14):")
print(pred_table)
write_csv(pred_table, here("results/tables", "paper2_h14_predicted_probabilities.csv"))
...
me_plot <- ggplot(pred_table, aes(x = NAGTargetsNonDemocracy, y = PredictedProbability,
  ...
  labs(title = "H14: Larger Domestic Opposition Increases NAG Support to Non-Democracy Targets (Bandwidth Interaction)",
...
ggsave(
  filename = here("results/plots", "paper2_h14_marginal_effects.png"),
  plot     = me_plot,
  width    = 10,
  height   = 7,
  dpi      = 300,
...
message("Plot saved directly: paper2_h14_marginal_effects.png")

# ----------------------------------------------------------------------------
# Aggressive cleanup
# ----------------------------------------------------------------------------
rm(list = setdiff(ls(), c("df_final", "model_h14_qp", "model_h14_nb")))
gc()
