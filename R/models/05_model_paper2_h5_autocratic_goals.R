# =============================================================================
# 05_model_paper2_h5_autocratic_goals.R
# Tier 1 Model for Paper 2 (Ideological Commitment Signaling)
# H5: Revisionist autocrats are more likely to support NAGs with autocratic goals
# =============================================================================
here::i_am("R/models/05_model_paper2_h5_autocratic_goals.R")

# Force clean load of trimmed + fixed data (Rule 15)
source(here("R/shared/04_trim_and_finalize.R"))

# ----------------------------------------------------------------------------
# Packages
# ----------------------------------------------------------------------------
library(fixest)
library(texreg)
library(ggplot2)
library(dplyr)

message("Modeling packages loaded. df_final ready: ", nrow(df_final), " rows")

# ----------------------------------------------------------------------------
# Prepare model data with friendly English labels (Rule 9)
# ----------------------------------------------------------------------------
df_model <- df_final |>
  drop_na(nags_support_count, revisionist_high, nags_nondem_objective,
          politicalbandwidth, ln_capital_dist_km, cinc_a_log, cinc_b_log) |>
  mutate(
    RevisionistLeaderHigh    = revisionist_high,
    NAGNonDemObjective       = nags_nondem_objective,
    PoliticalBandwidth       = politicalbandwidth,
    LogCapitalDistance       = ln_capital_dist_km,
    SenderCINCLog            = cinc_a_log,
    TargetCINCLog            = cinc_b_log
  )

# ----------------------------------------------------------------------------
# Primary model: Quasi-Poisson (committee recommendation)
# ----------------------------------------------------------------------------
model_h5_qp <- feglm(
  nags_support_count ~ RevisionistLeaderHigh * NAGNonDemObjective +
    PoliticalBandwidth + LogCapitalDistance +
    SenderCINCLog + TargetCINCLog + cold_war + war_on_terror,
  family = quasipoisson(link = "log"),
  cluster = ~dyad,
  data = df_model
)

# ----------------------------------------------------------------------------
# Robustness: Negative Binomial (with safety catch)
# ----------------------------------------------------------------------------
model_h5_nb <- tryCatch(
  fenegbin(
    nags_support_count ~ RevisionistLeaderHigh * NAGNonDemObjective +
      PoliticalBandwidth + LogCapitalDistance +
      SenderCINCLog + TargetCINCLog + cold_war + war_on_terror,
    cluster = ~dyad,
    data = df_model
  ),
  error = function(e) {
    message("Negative Binomial failed to converge - skipping for now.")
    NULL
  }
)

# ----------------------------------------------------------------------------
# Save tables (CSV + LaTeX) — no RDS models (Rule 4)
# ----------------------------------------------------------------------------
if (!is.null(model_h5_nb)) {
  etable(model_h5_qp, model_h5_nb,
         file = here("results/tables", "paper2_h5_quasipoisson_nb.tex"),
         replace = TRUE)
  coef_table <- etable(model_h5_qp, model_h5_nb, tex = FALSE)
} else {
  etable(model_h5_qp,
         file = here("results/tables", "paper2_h5_quasipoisson.tex"),
         replace = TRUE)
  coef_table <- etable(model_h5_qp, tex = FALSE)
}

write_csv(coef_table, here("results/tables", "paper2_h5_coefficients.csv"))
message("Tables saved.")

# ----------------------------------------------------------------------------
# Predicted counts table (Revisionist High × NAG Non-Dem Objective)
# ----------------------------------------------------------------------------
newdata <- expand.grid(
  RevisionistLeaderHigh  = c(0, 1),
  NAGNonDemObjective     = c(0, 1),
  PoliticalBandwidth     = mean(df_model$PoliticalBandwidth, na.rm = TRUE),
  LogCapitalDistance     = mean(df_model$LogCapitalDistance, na.rm = TRUE),
  SenderCINCLog          = mean(df_model$SenderCINCLog, na.rm = TRUE),
  TargetCINCLog          = mean(df_model$TargetCINCLog, na.rm = TRUE),
  cold_war               = 0,
  war_on_terror          = 0
)

preds <- predict(model_h5_qp, newdata = newdata, type = "response")

pred_table <- data.frame(
  RevisionistLeaderHigh  = rep(c("No", "Yes"), each = 2),
  NAGNonDemObjective     = rep(c("No", "Yes"), 2),
  PredictedCount         = round(preds, 4)
)

print("Predicted Counts Table (H5):")
print(pred_table)
write_csv(pred_table, here("results/tables", "paper2_h5_predicted_counts.csv"))
message("Predicted counts table saved.")

# ----------------------------------------------------------------------------
# Marginal effects plot — saved DIRECTLY to PNG (no viewer pane)
# ----------------------------------------------------------------------------
me_plot <- ggplot(pred_table, aes(x = NAGNonDemObjective, y = PredictedCount,
                                  color = RevisionistLeaderHigh, shape = RevisionistLeaderHigh)) +
  geom_point(size = 4) +
  geom_line(aes(group = RevisionistLeaderHigh), linewidth = 1) +
  labs(title = "H5: Revisionist Leaders Send More Support to NAGs with Non-Democratic Objectives",
       x = "NAG Has Non-Democratic Objective (0 = No, 1 = Yes)",
       y = "Predicted NAG Support Count",
       color = "Revisionist Leader (High)",
       shape = "Revisionist Leader (High)") +
  theme_minimal(base_size = 14)

ggsave(
  filename = here("results/plots", "paper2_h5_marginal_effects.png"),
  plot     = me_plot,
  width    = 10,
  height   = 7,
  dpi      = 300,
  units    = "in"
)

message("Plot saved directly: paper2_h5_marginal_effects.png")

# ----------------------------------------------------------------------------
# Aggressive cleanup
# ----------------------------------------------------------------------------
rm(list = setdiff(ls(), c("df_final", "model_h5_qp", "model_h5_nb")))
gc()
message("[05] Environment cleaned. Ready for next model.")