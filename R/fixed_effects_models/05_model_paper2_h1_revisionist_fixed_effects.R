...
# Primary: Logit with dyad FE | Robustness: Probit | Subtype logits
# =============================================================================

here::i_am("R/models/fixed_effects_models/05_model_paper2_h1_revisionist_fixed_effects.R")
...
# PRIMARY: Logit with dyad fixed effects
model_h1_logit <- feglm(
  nags_any_support ~ RevisionistLeader +
    PoliticalBandwidth + LogCapitalDistance +
    SenderCINCLog + TargetCINCLog + cold_war + war_on_terror | dyad,
  family = binomial(link = "logit"),
  cluster = ~ dyad,
  data = df_model
)

# Robustness: Probit with dyad fixed effects
...
etable(model_h1_logit, model_h1_probit,
       file = here("results/tables/paper2_h1_logit_probit.tex"), replace = TRUE)

etable(model_h1a_logit, model_h1b_logit, model_h1c_logit,
       file = here("results/tables/paper2_h1_subtypes.tex"), replace = TRUE)
...
ggsave(here("results/plots/paper2_h1_marginal_effects.png"),
       plot = me_plot, width = 10, height = 7, dpi = 300)

message("H1 logit models complete.")