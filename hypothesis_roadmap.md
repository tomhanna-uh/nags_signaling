...
| **H2** | NAG Support Count | sidea_revisionist_domestic × nags_ideology_match | Poisson/NegBin | log(Count) = β₀ + β₁·Revisionist + β₂·IdeologyMatch + β₃·(Revisionist × IdeologyMatch) + controls | Same as H1 | fixest, pscl (zeroinfl) | `fepoisson(nags_support_count ~ revisionist_high * nags_ideology_match_cont + ...)` | Zero-inflated, region FE, subsample nags>0 | Rational alignment test. |
